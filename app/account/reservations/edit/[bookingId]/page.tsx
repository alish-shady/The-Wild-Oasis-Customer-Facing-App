...
export default async function Page(props: PageProps<"/account/reservations/edit/[bookingId]">) {
  const { bookingId } = await props.params;
  const selectedBooking = await getBooking(Number(bookingId));
  const { maxCapacity } = await getCabin(selectedBooking.cabinId);
      ...
      <form action={updateReservation} className="bg-primary-900 py-8 px-12 text-lg flex gap-6 flex-col">
        <div className="space-y-2">
          ...
          <select
            ...
            defaultValue={selectedBooking.numGuests}
            required
          >
            <option value="" key="">
        ...
        <div className="space-y-2">
          ...
          <textarea
            name="observations"
            defaultValue={selectedBooking.observations || ""}
            className="px-5 py-3 bg-primary-200 text-primary-800 w-full shadow-sm rounded-sm"
          />
        </div>
        ...
        <input hidden type="hidden" readOnly name="bookingId" value={bookingId} />
      </form>
    </div>
  );
}
